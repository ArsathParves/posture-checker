...
- **D2** — Parent-delegation NS read in `dnsmod` uses the first resolver from the fallback list; if that resolver returns a lame answer the whole "RDAP vs parent" comparison is skewed. It should query at least two independent resolvers and require agreement.
- **D3** — `AXFR` probe timeout is fixed at ~5s; on high-latency links this both slows the run and produces `WARN: AXFR could not be tested` false alarms. Move the timeout to a config constant and make failure explicitly UNKNOWN.
- **D4** — DNSSEC chain validation (`dnsmod.dnssec_status`) requires `cryptography`; if the import fails, current behaviour is to grade "could not determine" (correct per CLAUDE.md). But the finding text does not tell the user *why* — it should read "cryptography module not installed; DNSSEC validation skipped" rather than a generic "could not retrieve".
- **D5** — `open_resolver_check` uses a single probe from the primary NS; a nameserver serving different responses for recursion queries depending on source subnet (increasingly common) can produce false PASS.
- **D6** — When TXT is truncated + TCP/53 is blocked, the SPF/DKIM/DMARC path already returns `TxtUnretrievable` (good, N2). But the `AXFR` check does not raise an equivalent — it returns a generic negative. Make it symmetric.

## Grading Issues

- **G1** — The correctness/hardening split (80/20) is not surfaced anywhere in the CLI or web UI. A user seeing "Overall: B" cannot infer whether correctness is A and hardening D, or vice versa. Print both sub-grades.
...
- **G3** — The `google.com` guard-rail (must grade **A overall** despite DNSSEC absent) rests on `HARDENING_ABSENCE` catching `"DNSSEC status"`. It works today. No test pins this. Add `tests/test_grading_google_unsigned.py` that fakes a report matching google.com's live shape and asserts overall == A.
