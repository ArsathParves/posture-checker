...
|---|---|---|
| T1 | DONE | Stable `finding_id` field added to `Finding` / `Report.add` / `_f2d` wire format. Display-independent handle for external consumers (JSON, SPA, remediation lookups). Grading remains driven by status + hardening + severity — `finding_id` is orthogonal (regression test locks this in). Default is `""` (empty string, not `None` — SPA reads `.length`). Mechanism-first, same pattern as T2/T5: field ships; migration of specific emit sites to declare stable IDs is follow-up. Pinned by `tests/test_finding_id_mechanism.py` (10); wire-format golden fixture extended to exercise a migrated emit site (`DNSSEC_CHAIN_BROKEN`). |
...
| T4 | DONE | `tests/test_grading_ground_truth.py` pins grade outcomes for cloudflare.com / dnssec-failed.org / vergecloud.com; `tests/test_grading_google_unsigned.py` pins google.com. Wire-format golden landed as `tests/test_report_golden_shape.py` (5 cases: top-level key set, per-finding key set, whole-report byte-comparable golden, insertion-order preservation, string-typed values). Trip-wire against silent schema drift — every new field on `Finding` or every rename in `_f2d` must update the golden literal deliberately. |
...
| B5 | DONE | Correctness/hardening split surfaced in CLI (`tests/test_grade_split_surfaced.py`) and hardening-gaps caption (`tests/test_hardening_caption.py`). `grade()` now returns a `grade_reconciliation` dict naming the 0.8/0.2 correctness/hardening weights, the CRITICAL-floors-to-F rule, per-bucket finding counts, and a `sections_note` that per-section grades are informational and not aggregable — every renderer reads the same source of truth. Pinned by `tests/test_grade_reconciliation.py` (9 cases). CLI/web UI use of the reconciliation is follow-up (mechanism-first, same shape as T5). |
...
| B30 | DONE | `posture/tlsprobe.py` + `_tls_posture` section emit Certificate expiry (7d/30d thresholds), TLS protocol version (TLS 1.2/1.3 PASS; 1.0/1.1 FAIL per RFC 8996 + PCI-DSS), HSTS (6-month baseline). Rule-5-gated on `safe_for_direct_dns`; UNKNOWN row when the path is untrusted. **CAA-vs-served-issuer alignment (RFC 8659 §3) landed**: `_caa_identifier_matches_issuer` + `_CA_ALIASES` translate CAA labels (`letsencrypt.org`, `comodoca.com`) to the fuzzy issuer strings CAs actually serve; sub-CA CNs (R3/E1) match via the org field; rebrands (Comodo→Sectigo) handled by the alias table; unknown CAs fall back to a leading-label stem match. WARN emits `confidence=low` (the mapping is inherently fuzzy); `issue ";"` lockdown while a cert is served is FAIL/CRITICAL. Rule 1 boundaries: no CAA / only iodef → no finding; TLS probe failed → UNKNOWN; env unsafe → suppressed. Pinned by `tests/test_tls_posture.py` (18) + `tests/test_caa_issuer_alignment.py` (9). |
...
| B33 | DONE | `posture/selftest.py` — the SPOF is gone. The control leg no longer resolves `ns1.google.com` (removing both the resolver-lookup dependency and the single-NS dependency); instead a random 3-of-13 sample of IANA root-server IPs (`_ROOT_SERVERS_V4`) is probed on every invocation. AA-flag trust requires a quorum (`_CONTROL_PROBE_MIN_AA_SUCCESS=2`), so a single misbehaving root cannot regress the check. TCP/53 reachability now traverses the same sample. Rule 5 semantics (`safe_for_per_ns_checks = not intercepted and aa_flag_trustworthy`) preserved; return-shape contract unchanged. Pinned by `tests/test_selftest_no_spof_control.py` (7 cases: table shape + rotation + no-resolver + one-root-down + all-down + partial success + AA stripping) plus `tests/test_selftest_check_environment.py` (10 existing cases, updated to lock in that a broken resolver no longer disables the control leg). |
